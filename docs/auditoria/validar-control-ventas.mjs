  ...
  for (const location of locs) {
    ...
    for (let i = 0; i < days.length; i += MAX_DAYS) {
      ...
      for (const d of r.days) {
        ...
        if (Math.round((m.precio.amount + m.lineaImporte.amount + m.cantidad.amount + m.sinFicha.amount + m.fichaDecimales.amount + m.sinCantidad.amount + m.sinPrecioUnitario.amount + m.redondeo + m.unidades.amount) * 100) / 100 !== m.diferencia) bad(`${location} ${d.day}: las partes no suman la diferencia`)
        ...
        if (m.sinCantidad.n) causas.sinCantidad = (causas.sinCantidad || 0) + 1
        if (m.sinPrecioUnitario.n) causas.sinPrecioUnitario = (causas.sinPrecioUnitario || 0) + 1
        ...
        if (process.env.DETALLE && (m.precio.n || m.lineaImporte.n || m.cantidad.detalle.length || m.sinFicha.n || m.fichaDecimales.detalle.length || m.sinCantidad.n || m.sinPrecioUnitario.n || m.unidades.grupos.length || m.redondeo)) for (const l of moneyLines(m, d.day)) console.log(`   ${location} ${d.day} | ${l}`)
      }
